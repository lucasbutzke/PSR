/*
 * Realizar contagem de tempo concorrentemente entre duas tarefas,
 * um tarefa faz um PRINT a cada 100ms e outra a cada 500ms
 *
...
void rtos_delay(uint32_t delay_in_ms){	// travar a thread por um tempo (ms) de execucao
	...
	while(xTaskGetTickCount() < current_tick_count + delay_in_ticks); // vTaskDelay(100); // para melhor preemptar as tarefas
}
